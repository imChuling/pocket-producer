...
| C1e | Fusion gain replicates on deployed MS-CLAP space: hard\_similar 0.716→0.768 (+5.2pp), overall 0.864→0.877, both 5/5 seeds (one-sided sign test p=0.031). All 6 conditions ran, but role\_gap was UNTESTED in this space (implementation gap: LAION text prompts incompatible with 1024-dim embeddings; feature constant 0, so its zero contribution to "fusion-7sig" is arithmetic, not evidence). The harmonic null does replicate: "cosine+session" here = cosine+harmonic, 0.8625 vs cosine 0.8638, no gain | `artifacts/fusion-msclap/fusion.json` | verified 2026-08-07 |
