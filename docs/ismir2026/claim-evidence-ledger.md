...
| E3b | MS-CLAP sensitivity re-analysis (same split, deployed embedding space): hard\_similar +9.2pp, CI [+4.3, +14.3] entirely above zero; hard\_tempo\_key −1.8pp, CI [−4.5, +0.6] crosses zero; overall +2.0pp, CI [+0.6, +3.5]. 7.7pp swing from LAION-CLAP is evidence of representation sensitivity. LOSO: tempo −6.9/+0.7pp (direction holds); key −0.4pp on hard\_similar (reverses from +0.9pp). Training recipe also differs (source-weighted BPR, hard\_similar×2.0); dev-set 2×2 factorial decomposes fusion-cosine delta on hard\_similar: representation +1.9pp, training +0.5pp, interaction +0.3pp — primarily representation-driven (§4.2–4.3) | `artifacts/msclap-sensitivity/sensitivity.json`, protocol `research/protocol-msclap-sensitivity-v1.md` (frozen at `3869af8`); `artifacts/factorial/factorial.json`, protocol `research/protocol-factorial-v1.md` (frozen at `f156c75`) | verified 2026-08-10 |
| E4 | Residual limitation: signal set/hyperparameters designed on earlier mixed-label corpus; pack-only corpus is a subset of previously inspected data; LAION-CLAP run is sensitivity re-analysis, not de novo confirmation; MS-CLAP sensitivity changed both embedding and training recipe, but dev-set factorial shows representation is dominant (+1.9pp vs +0.5pp training, +0.3pp interaction) (§4.3) | stated in paper and in `research/protocol-heldout-v2.md` + `research/protocol-msclap-sensitivity-v1.md` + `artifacts/factorial/factorial.json` | acknowledged |
