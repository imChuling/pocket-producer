...
| E3b | MS-CLAP sensitivity re-analysis (same split, deployed embedding space): hard\_similar +9.2pp, CI [+4.3, +14.3] entirely above zero; hard\_tempo\_key −1.8pp, CI [−4.5, +0.6] crosses zero; overall +2.0pp, CI [+0.6, +3.5]. 7.7pp swing from LAION-CLAP is evidence of representation sensitivity. LOSO: tempo −6.9/+0.7pp (direction holds); key −0.4pp on hard\_similar (reverses from +0.9pp). Training recipe also differs (source-weighted BPR, hard\_similar×2.0); dev-set 2×2 factorial decomposes fusion-cosine delta on hard\_similar: representation +1.9pp, training +0.5pp, interaction +0.3pp — primarily representation-driven (§4; paper shows headline +9.2pp/CI/7.7pp only, detail in artifacts) | `artifacts/msclap-sensitivity/sensitivity.json`, protocol `research/protocol-msclap-sensitivity-v1.md` (frozen at `3869af8`); `artifacts/factorial/factorial.json`, protocol `research/protocol-factorial-v1.md` (frozen at `f156c75`) | verified 2026-08-10 |
| E4 | Residual limitation: signal set/hyperparameters designed on earlier mixed-label corpus; pack-only corpus is a subset of previously inspected data; LAION-CLAP run is sensitivity re-analysis, not de novo confirmation; MS-CLAP sensitivity changed both embedding and training recipe, but dev-set factorial shows representation is dominant (+1.9pp vs +0.5pp training, +0.3pp interaction) (§4) | stated in paper and in `research/protocol-heldout-v2.md` + `research/protocol-msclap-sensitivity-v1.md` + `artifacts/factorial/factorial.json` | acknowledged |
| E5 | Pilot: 9 raters, 20 blind A/B pairs, Fleiss' κ=0.294; 1 pair excluded ("neither" majority), 9 of the remaining 19 preference pairs at ≥7/9 consensus; harmonic matches majority 7/9, tempo 3/9 — descriptive divergence, post-hoc, different corpus (§4; paper shows headline κ only, detail in artifact) | `artifacts/human-signal-alignment/alignment.json` (n\_pairs=20, n\_preference=19, n\_consensus=9) | verified 2026-08-10 (exclusion disclosed per review) |
...
| 2026-08-10 | Claude Code | MS-CLAP sensitivity re-analysis: source-weighted BPR 训练+三基线评测、部署权重重导出、论文双空间对比更新、number checker 扩展(151 checks)、claim-evidence ledger 同步;2×2 factorial 实验脚本+运行(representation×training, dev-set 5-fold CV)、confound caveat 替换为 factorial 分解(+1.9/+0.5/+0.3pp)、checker 扩展至 161 checks;代码库清理(死代码/废弃 artifacts/研究数据 ~8.8GB);论文重构为系统导向叙事(§2 扩展至 4 小节、§4 压缩为无子节段落、LOSO 表删除)、checker 更新至 97 checks;所有实验设计与结论由作者核验并负责 |
