...
surfaced for preview---together with per-signal evidence (audio
similarity, tempo fit, tag overlap)---before any session state
changes.
...
Still, a single fused score averages over regimes in which
individual signals switch sign, so its ranking cannot be uniformly
trusted.
...
candidate generator: on the held-out split it performs comparably to
frozen cosine overall ($+0.4$\,pp, CI crossing zero), trading a
positive but uncertain point estimate on hard\_similar ($+1.5$\,pp,
4/5 seeds) against a significant deficit on hard\_tempo\_key---and,
unlike a single opaque cosine score, it surfaces per-signal evidence
with each proposal.
Because its signal contributions switch sign across regimes and its
rankings may not reflect human preference, the proxy never decides
alone: every recommendation is previewable and reversible.
